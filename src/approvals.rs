...
pub fn verify(token: &ApprovalToken, verifying_key_b64: &str) -> bool {
  ...
  if token.payload.expires_at_unix < now { return false; }

  let maybe_vk = if verifying_key_b64.trim().is_empty() { derive_verifying_from_env() } else {
    general_purpose::STANDARD.decode(verifying_key_b64).ok()
      .and_then(|b| VerifyingKey::from_bytes(&b.try_into().ok()?).ok())
  };

  if let Some(vk) = maybe_vk {
    let sig_bytes = match general_purpose::STANDARD.decode(&token.sig_b64) { Ok(b) => b, Err(_) => return false };
    let sig = match Signature::from_slice(&sig_bytes) { Ok(s) => s, Err(_) => return false };
    return vk.verify_strict(&canonical_payload_bytes(&token.payload), &sig).is_ok();
  }
...
pub async fn sign_dev_approval(State(st): State<AppState>, Json(req): Json<DevSignReq>) -> (StatusCode, Json<serde_json::Value>) {
  let enabled = std::env::var("AEGIS_DEV_SIGNER").unwrap_or_else(|_| "0".to_string()) == "1";
  if !enabled {
    return (StatusCode::NOT_FOUND, Json(serde_json::json!({"error":"dev signer disabled"})));
  }

  let sk_b64 = match std::env::var("AEGIS_OPERATOR_SK_B64") {
    Ok(v) => v,
    Err(_) => return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error":"AEGIS_OPERATOR_SK_B64 missing"}))),
  };

  let sk_bytes = match general_purpose::STANDARD.decode(sk_b64) {
    Ok(b) => b,
    Err(_) => return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error":"bad operator sk b64"}))),
  };
  if sk_bytes.len() != 32 {
    return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error":"operator sk must be 32 bytes"})));
  }
  ...
  let payload = ApprovalPayload { intent_hash: req.intent_hash, policy_hash: req.policy_hash, expires_at_unix: expires, scope: req.scope };
  let sig: Signature = sk.sign(&canonical_payload_bytes(&payload));
  let tok = ApprovalToken { payload, sig_b64: general_purpose::STANDARD.encode(sig.to_bytes()) };

  st.ledger.append("approval.sign", "DEV", serde_json::json!({"scope": tok.payload.scope, "expires": tok.payload.expires_at_unix}));
